...
const MAX_ADDITIONAL_LENGTH = 500;
const MAX_KEYS = 16;
...
export function allowedEquipmentSpecKeys(fields: EquipmentSpecField[]): Set<string> {
...
export function sanitizeEquipmentSpecifications(
  input: unknown,
  ...categoryValues: Array<string | null | undefined>
): { specs: EquipmentSpecifications | null; error?: string } {
  if (input == null) return { specs: null };
  if (typeof input !== 'object' || Array.isArray(input)) {
    return { specs: null, error: 'Specifications must be an object.' };
  }

  const fields = getEquipmentSpecFields(...categoryValues);
  const allowed = allowedEquipmentSpecKeys(fields);
  const entries = Object.entries(input as Record<string, unknown>);
  if (entries.length > MAX_KEYS) {
    return { specs: null, error: 'Too many specification fields.' };
  }

  const result: EquipmentSpecifications = {};
  for (const [key, raw] of entries) {
    if (FORBIDDEN_KEYS.has(key) || key.includes('.')) {
      return { specs: null, error: 'Invalid specification field.' };
    }
    if (!allowed.has(key)) {
      return { specs: null, error: `Unexpected specification field: ${key}.` };
    }
    ...
    if (field.type === 'number') {
      ...
      if (!Number.isFinite(numeric) || numeric <= 0) {
        return { specs: null, error: `${field.label} must be a positive number.` };
      }
      result[key] = numeric;
      continue;
    }
    ...
    if (typeof raw !== 'string' && typeof raw !== 'number') {
      return { specs: null, error: `${field.label} is invalid.` };
    }
    const text = String(raw).trim();
    if (!text) continue;
    const max = key === 'additional' ? MAX_ADDITIONAL_LENGTH : MAX_STRING_LENGTH;
    if (text.length > max) {
      return { specs: null, error: `${field.label} is too long.` };
    }
    if (field.options && !field.options.includes(text)) {
      return { specs: null, error: `${field.label} must be one of: ${field.options.join(', ')}.` };
    }
    result[key] = text;
  }

  return { specs: Object.keys(result).length ? result : null };
}
